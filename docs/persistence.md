# OuterBrain Workspace Persistence

## Scope

OuterBrain Workspace owns provider-neutral semantic runtime, durable semantic session, and publication orientation. This document is package-local and is the persistence contract for `.` in `outer_brain`.
...
Memory/ref-only semantic posture by default; raw Ecto/Postgres persistence is explicit in outer_brain_persistence.
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. :integration_postgres through outer_brain_persistence and package-local migration proof.
...
outer_brain_persistence owns restart-critical durable tables.
