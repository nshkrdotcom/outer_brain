# OuterBrain Outer Brain Contracts Persistence

## Scope

OuterBrain Outer Brain Contracts owns OuterBrain semantic, memory, authority, runtime, or restart persistence evidence owner. This document is package-local and is the persistence contract for `core/outer_brain_contracts` in `outer_brain`.
...
```bash
cd core/outer_brain_contracts && mix test; root mix ci
```
