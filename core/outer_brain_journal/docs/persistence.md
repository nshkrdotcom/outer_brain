# OuterBrain Outer Brain Journal Persistence

## Scope

OuterBrain Outer Brain Journal owns OuterBrain semantic, memory, authority, runtime, or restart persistence evidence owner. This document is package-local and is the persistence contract for `core/outer_brain_journal` in `outer_brain`.
...
```bash
cd core/outer_brain_journal && mix test; root mix ci
```
