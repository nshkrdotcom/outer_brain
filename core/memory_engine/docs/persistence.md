# OuterBrain Memory Engine Persistence

## Scope

OuterBrain Memory Engine owns OuterBrain semantic, memory, authority, runtime, or restart persistence evidence owner. This document is package-local and is the persistence contract for `core/memory_engine` in `outer_brain`.
...
```bash
cd core/memory_engine && mix test; root mix ci
```
