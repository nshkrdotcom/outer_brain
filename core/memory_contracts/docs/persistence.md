# OuterBrain Memory Contracts Persistence

## Scope

OuterBrain Memory Contracts owns OuterBrain semantic, memory, authority, runtime, or restart persistence evidence owner. This document is package-local and is the persistence contract for `core/memory_contracts` in `outer_brain`.
...
```bash
cd core/memory_contracts && mix test; root mix ci
```
