defmodule OuterBrain.Memory.SidecarIndex do
  @moduledoc """
  Process-local index from sidecar state to fragment references.
  """

  @table_name :outer_brain_sidecar_index
  ...
  def put(index, key_attrs, fragments) when is_list(fragments) do
    with {:ok, key} <- normalize_key(key_attrs),
      ...
      true = :ets.insert(index, {cache_tuple(key), entry})
      :ok
    end
  end

  @spec fetch(:ets.tid(), map()) :: {:ok, [map()]} | :miss | {:error, term()}
  def fetch(index, key_attrs) do
    with {:ok, key} <- normalize_key(key_attrs) do
      case :ets.lookup(index, cache_tuple(key)) do
      ...
      nil ->
        {:ok,
         %{
           ...
           sidecar_ref: fetch_value(attrs, :sidecar_ref)
         }}

      field ->
        {:error, {:missing_sidecar_key, field}}
    end
  end

  defp cache_tuple(key), do: {key.tenant_ref, key.snapshot_epoch, key.sidecar_ref}
  ...
  defp sanitize_fragment_ref(fragment) when is_map(fragment) do
    cond do
      has_payload?(fragment, :content) ->
        {:error, {:unsafe_sidecar_payload, :content}}
  ...
  defp evict_matching(index, invalidation) do
    index
    |> :ets.tab2list()
