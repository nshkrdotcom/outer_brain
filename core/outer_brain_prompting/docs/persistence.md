# OuterBrain Outer Brain Prompting Persistence

## Scope

OuterBrain Outer Brain Prompting owns OuterBrain semantic, memory, authority, runtime, or restart persistence evidence owner. This document is package-local and is the persistence contract for `core/outer_brain_prompting` in `outer_brain`.
...
```bash
cd core/outer_brain_prompting && mix test; root mix ci
```
