defmodule OuterBrain.RestartAuthority.RestartScan do
  ...
  def reconstruct(session_id, causal_unit_id, opts \\ [])
    ...
    publication_phase = store.latest_publication_phase(causal_unit_id, store_opts)
    analysis(session_id, causal_unit_id, pending_tasks, publication_phase)
  end

  @spec analysis(String.t(), String.t(), [struct()], :final | :provisional | nil) :: map()
  def analysis(session_id, causal_unit_id, pending_tasks, publication_phase)
